...
class trainer(nn.Module):
	...
	def train_network(self, args):
		...
		nloss = 0  # 全体の損失
		nloss_single = 0  # Singleモードの損失
		nloss_mom = 0  # MoMモードの損失
		...
		for num, (audio1, mixture1_lip_crops, audio2, mixture2_lip_crops) in enumerate(args.trainLoader, start = 1):
			...
			with autocast():				
				...
				if mode == "single":
					# Singleモード: estim2を0に近づける（L2損失）
					loss2 = torch.mean(estim2 ** 2)  # L2損失
					loss = loss1 + loss2
			...
			single_loss_str = f"Single: {nloss_single/n_single:.3f}" if n_single > 0 else "Single: N/A"
			mom_loss_str = f"MoM: {nloss_mom/n_mom:.3f}" if n_mom > 0 else "MoM: N/A"
